...
fn main() -> Result<()> {
    ...
    eprintln!(
        "globacl-control listening on {bind_addr}; data_dir={}; node_id={}; role={}; term={}; quorum={}/{}",
        data_dir.display(),
...
fn prepare_on_quorum(app: &App, mutation: &Mutation) -> Result<()> {
    ...
    Err(GlobAclError::InvalidData(format!(
        "control quorum unavailable: prepared={prepared} quorum={} failures={}",
        app.replication.quorum,
        failures.join(",")
    )))
}
...
fn control_sync_loop(app: Arc<App>, interval: Duration) {
    loop {
        if let Err(err) = sync_from_leader(&app) {
            eprintln!("control peer sync failed: {err}");
...
fn replication_config(bind_addr: &str) -> Result<ReplicationConfig> {
    let node_id = env::var("GLOBACL_CONTROL_NODE_ID")
        .or_else(|_| env::var("HOSTNAME"))
        .unwrap_or_else(|_| "control-local".to_owned());
    let cluster_id = env::var("GLOBACL_CONTROL_CLUSTER_ID").unwrap_or_else(|_| "local".to_owned());
    let initial_leader_id = env::var("GLOBACL_CONTROL_INITIAL_LEADER_ID")
        .or_else(|_| env::var("GLOBACL_CONTROL_LEADER_ID"))
        .ok()
        .filter(|value| !value.trim().is_empty());
    let mut peers = parse_peers(
        env::var("GLOBACL_CONTROL_PEERS")
            .unwrap_or_default()
            .as_str(),
    )?;
    if peers.is_empty() {
    ...
    if let Some(leader_id) = &initial_leader_id {
        if !peers.iter().any(|peer| &peer.node_id == leader_id) {
            return Err(GlobAclError::InvalidData(format!(
                "initial leader {leader_id} is not present in GLOBACL_CONTROL_PEERS"
            )));
        }
    }
    let quorum = env::var("GLOBACL_CONTROL_QUORUM")
        .ok()
        .map(|value| parse_env_usize(&value, "GLOBACL_CONTROL_QUORUM"))
        .transpose()?
    ...
    let heartbeat_interval_ms = env::var("GLOBACL_CONTROL_HEARTBEAT_MS")
        .ok()
        .map(|value| parse_env_u64(&value, "GLOBACL_CONTROL_HEARTBEAT_MS"))
        .transpose()?
        .unwrap_or(250);
    let election_timeout_ms = env::var("GLOBACL_CONTROL_ELECTION_MS")
        .ok()
        .map(|value| parse_env_u64(&value, "GLOBACL_CONTROL_ELECTION_MS"))
        .transpose()?
        .unwrap_or(1200);
    let sync_interval_ms = env::var("GLOBACL_CONTROL_SYNC_MS")
        .ok()
        .map(|value| parse_env_u64(&value, "GLOBACL_CONTROL_SYNC_MS"))
        .transpose()?
        .unwrap_or(1000);
