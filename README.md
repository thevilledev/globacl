...
- `globacl-core`: domain model, idempotent source-of-truth state, exact lookup engine, binary mutation stream, binary snapshot format, per-shard append log helpers.
- `globacl-control`: ACL authoring/API service with a linearized in-process source of truth and durable per-shard append logs.
- `globacl-relay`: regional relay that proxies mutation and snapshot fetches from control.
- `globacl-agent`: PoP agent that cold-starts from a snapshot, polls deltas through the relay, applies exact local state, and exposes local lookup.
